...
func (a *App) layoutStatusBar(gtx layout.Context, r image.Rectangle) {
	...
	// Status text row.
	{
		defer op.Offset(image.Pt(gtx.Dp(unit.Dp(8)), hintsH+gtx.Dp(unit.Dp(4)))).Push(gtx.Ops).Pop()
		col := a.Th.Text
		...
		a.drawStatusText(gtx, col, a.State.StatusText)
	}

	// Right-side: team / filter context.
	right := ""
	if a.State.Team != nil {
	...
	if right != "" {
		rw := a.statusTextWidth(gtx, right) + gtx.Dp(unit.Dp(16))
		defer op.Offset(image.Pt(w-rw, hintsH+gtx.Dp(unit.Dp(4)))).Push(gtx.Ops).Pop()
		a.drawStatusText(gtx, a.Th.AccentDim, right)
	}
}
...
func (a *App) openSettings() {
	...
	if st.ShowSettings && st.Settings == nil {
		st.Settings = NewSettingsModal(a.Th)
	}
	a.updateHints()
}
